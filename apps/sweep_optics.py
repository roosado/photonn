...
def stack_wraparound(z: float, n_hops: int, fields: np.ndarray, *, pad: int = 3):
...
def geometry_row(z_mm: float, layers: int, fields: np.ndarray) -> dict:
    z = z_mm * 1e-3
    hops = layers + 1
    ...
    row.update(stack_wraparound(z, hops, fields))
    return row


def probe_fields(n: int = 16) -> np.ndarray:
...
def candidate_geometry(args, fields):
    ...
    """
    if args.arm == "layers":
        return [geometry_row(args.z, layers, fields) for layers in LAYER_LIST]
    if args.arm == "iso":
        return [geometry_row(z, L, fields) for z, L in iso_reach_configs()]
    return [geometry_row(z_mm, args.layers, fields) for z_mm in Z_LIST_MM]


def report_geometry(args, fields=None):
    fields = probe_fields() if fields is None else fields
    z_crit = GRID * DX**2 / WAVELENGTH
    if args.arm == "iso":
        varying, fixed = "the z/L split", f"total reach {ISO_REACH_PX:.1f} px"
    elif args.arm == "layers":
        varying, fixed = "mask count", f"z = {args.z:g} mm"
    else:
        varying, fixed = "separation", f"{args.layers} masks"
    print(f"grid {GRID}, dx {DX*1e6:g} um, lambda {WAVELENGTH*1e9:g} nm | varying {varying}, {fixed}")
    print(f"z_crit = {z_crit*1e3:.3f} mm   reach/hop = z x {WAVELENGTH/(2*DX**2):.1f} px/m\n")
    print(f"{'z (mm)':>7} {'masks':>6} {'reach/hop':>10} {'total':>8} {'sampling':>9} "
          f"{'1-hop wrap':>11} {'stack wrap':>11} {'logit err':>10} {'flips':>7}")

    rows = candidate_geometry(args, fields)
    for r in rows:
        print(f"{r['z_mm']:>7.1f} {r['layers']:>6} {r['reach_px_per_hop']:>10.2f} "
              f"{r['reach_px_total']:>8.1f} {'ok' if r['sampling_ok'] else 'ALIAS':>9} "
...
def _write(path, update: dict):
    ...
    keys = {"runs": lambda r: r["config"], "geometry": lambda r: (r["z_mm"], r["layers"])}
...
def run_config(z_mm: float, layers: int, train_ds, val_ds, args) -> dict:
    z = z_mm * 1e-3
    tag = f"z{z_mm:g}mm_L{layers}"
    model = D2NN(n=GRID, n_layers=layers, dx=DX, wavelength=WAVELENGTH,
    ...
    return {
        "config": tag, "z_mm": z_mm, "layers": layers, "n_params": n_params,
...
def main():
    ...
    if args.arm == "z":
        configs = [(z, args.layers) for z in Z_LIST_MM if (z, args.layers) in clean]
        configs.insert(0, (args.z, 0))          # pure-diffraction floor, free
    elif args.arm == "iso":
        configs = [c for c in iso_reach_configs() if c in clean]
    else:
