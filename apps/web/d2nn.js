...
(function () {
  ...
  if (!ASM) throw new Error("d2nn.js needs asm.js loaded first");

  /**
   * Build a network around one weights bundle.
   *
   * The decoded masks, their precomputed cos/sin and the transfer function all
  ...
  const DEFAULT_W = (typeof window !== "undefined" && window.D2NN_WEIGHTS) ? window.D2NN_WEIGHTS
    : (typeof require !== "undefined" ? require("./d2nn_weights.js") : null);
  const NET = DEFAULT_W ? buildNet(DEFAULT_W) : {};
  NET.buildNet = buildNet;
